...
- [x] 修复 `--check` 对 `create` / `link` 可写性预检的假阳性
- [x] 清理文档中的本机绝对路径引用
...
- [x] 为 release workflow 增加发版前测试门禁

## 当前分析任务

- [x] 生成项目结构分层结果 `layers.json`
 - [x] 校验 assembled graph 完整性并生成 `review.json`
...
- [x] 修复受保护符号链接在 `relink` 下的确认绕过
- [x] 修正窄终端下审阅界面的顶部和底部换行
- [x] 补齐 `link` / `clean` / `create` 的关键测试缺口
- [x] 同步 `DESIGN.md` 到当前 CLI 与确认交互语义
...
- [x] 为入口和配置加载链路补充更详细的块级注释
- [x] 为 runner 的风险确认与模式分流补充解释性注释
- [x] 为 create/link/clean 的关键边界分支补充说明
- [x] 为 output 和 TUI 的数据流与视图职责补充说明
- [x] 补充一份按阅读顺序组织的代码导图文档
